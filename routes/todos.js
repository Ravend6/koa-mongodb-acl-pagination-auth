...
router.get('edit', '/todos/:id/edit', policy.isAllowed(), function * (next) {
  yield this.render('todos/edit', {
    todo: this.todo
  });
});

router.put('/todos/:id/edit', policy.isAllowed(), function * (next) {
  try {
...
router.delete('delete', '/todos/:id', policy.isAllowed(), function * (next) {
  try {
